...
Behind the scenes {%= name %} uses [gana][] which in turns use [gana-compile][]. So the footprint and codebase is very small (**~1-2kb** minified and not gzipped), easy (just sync and async `.compile` and `.render` methods) and very well tested (this one has **~25 tests**).
...
```js
const es6template = require('{%= name %}')
```

{%= apidocs('index.js') %}

${
  verb.related && verb.related.list && verb.related.list.length
    ? related(verb.related.list, { word: 12 })
    : ''
}$
